...
def spread():             # 미세먼지를 확산시키는 함수
    ...
    for i in range(R):
        for j in range(C):       # 모든 행/열 확인
            ...
            for k in range(4):   # 미세먼지가 있는 경우 4 방향 수행
                nx = dx[k] + i
                ny = dy[k] + j
...
def air_up():          # 공기청정기를 위쪽으로 이동시키는 함수
    ...
    while True:
        ...
        if x == total_dust[0] and y == 0:
            break
        if nx < 0 or nx >= R or ny < 0 or ny >= C:   # 범위에서 벗어나지 않아야 함
            direct += 1    # 방향 카운트
            continue

        A[x][y], current = current, A[x][y]   # 공기 순환
        x = nx
        y = ny

def air_down():      # 공기청정기 아래쪽으로 이동시키는 함수
    # 시계 방향 공기 순환 -> 동 - 남 - 서 - 북
    dx = [0, 1, 0, -1]  # 이동방향 X
    dy = [1, 0, -1, 0]  # 이동방향 Y
    ...
    while True:
        nx = x + dx[direct]
        ny = y + dy[direct]
        if x == total_dust[1] and y == 0:
            break
        if nx < 0 or nx >= R or ny < 0 or ny >= C:
            direct += 1
            continue

        A[x][y], current = current, A[x][y]
        x = nx
        y = ny
