...
def rotate():
    for k in range(min(N, M) // 2):  # 범위 : 사각형 수
        ...
        for j in range(k + 1, N - k):  # 왼쪽 (X값 +, Y 고정)
            x = j  # x 좌표가 j이면
            former = A[x][y]  # 이전 좌표를 former에 저장
            A[x][y] = tmp  # A에 바꾼 좌표 저장
            tmp = former  # 바꾼 좌표를 이전 좌표에 저장

        for j in range(k + 1, M - k):  # 아래쪽 (X값 고정, Y값 +)
            y = j
            former = A[x][y]
            A[x][y] = tmp
            tmp = former

        for j in range(k + 1, N - k):  # 오른쪽 (X값 -, Y값 고정)
            x = N - j - 1
            former = A[x][y]
            A[x][y] = tmp
            tmp = former

        for j in range(k + 1, M - k):  # 위쪽 (X값 고정, Y값 -)
            y = M - j - 1
            former = A[x][y]
            A[x][y] = tmp
            tmp = former
